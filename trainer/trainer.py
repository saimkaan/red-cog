...
class Trainer(commands.Cog):
    ...
    async def fetch_data(self):
        while True:
            try:
                ...
                if token_ids:
                    self.fetch_trainer_data(token_ids)
                await asyncio.sleep(30)  # Run every 30 seconds
            except Exception as e:
                logging.error(f"Error occurred while fetching data: {e}")
                await asyncio.sleep(60)

    def fetch_trainer_data(self, token_ids):
        with self.executor as executor:
            future_to_token_id = {executor.submit(self.fetch_trainer_data_single, token_id): token_id for token_id in token_ids}
            for future in concurrent.futures.as_completed(future_to_token_id):
                token_id = future_to_token_id[future]
                try:
                    trainer_data = future.result()
                    if trainer_data and not self.is_message_sent_recently(token_id):
                        self.post_message(trainer_data)
                        self.update_last_message_time(token_id)
                except Exception as e:
                    logging.error(f"Error occurred while fetching Trainer data for token ID {token_id}: {e}")

    def fetch_trainer_data_single(self, token_id):
        try:
            ...
            logging.error(f"Error occurred while fetching data from Trainer API for token ID {token_id}: {e}")
        return None
    ...
    async def post_message(self, trainer_data):
        blur_link = f"https://blur.io/asset/0x8a3749936e723325c6b645a0901470cd9e790b94/{trainer_data['token_id']}"

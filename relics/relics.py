...
class TrainerRelics(commands.Cog):
    ...
    async def get(self, ctx, token_id: int):
        await self.fetch_and_print_trainer_data(ctx, token_id)

    async def fetch_and_print_trainerrelics_data(self, ctx, token_id):
        trainerrelics_data = self.fetch_trainerrelicsr_data(token_id)
        if trainer_data:
    ...
    def fetch_trainerrelics_data(self, trainerrelics_id):
        try:
            ...
            response = requests.post(self.url_trainerrelics, json=payload)
            data = response.json()
